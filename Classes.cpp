...
Stack<stacktype>::Stack(int count):size(count),last(0){
//	 size=count;
...
template <class Q>
Queue<Q>::Queue(int max_n):n(max_n),i(0),j(0){
	p=new int[max_n];
//	 n=max_n;
//	 i=j=0;
}
...
template <class A>
Array<A>::Array(int n):vol(n),max_vol(n+3){
//	 vol=n;
//	 max_vol=vol+3;
	mem=new int[max_vol];
}
...
/*
Spisok::Spisok():nodes(0),end(nullptr),begin(nullptr){
//	 nodes=0;
//	 end=begin=nullptr;
}
*/

Spisok::Spisok(int val):nodes(1){
//	 nodes=1;
	node *cur=new node;
	cur->obj=val;
	cur->next=nullptr;
